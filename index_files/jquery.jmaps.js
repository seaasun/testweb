...
Mapifies.MapObjects.Set = function(element, options) {
    ...
    var thisMap = new google.maps.Map(element);
...
Mapifies.Initialise = function(element, options, callback) {
    ...
    */
    ...
    if (true) {
        ...
        //thisMap.setCenter(new google.maps.LatLng(options.mapCenter[0], options.mapCenter[1]), options.mapZoom, options.mapType);
    ...
    */
    ...
    switch (options.centerMethod) {
        case 'normal':
            ...
            if(options.mapZoom!=undefined && options.mapZoom!=null){
				thisMap.setZoom(options.mapZoom);
            }
            break;
        case 'pan':
...
Mapifies.CreateKeyboardHandler = function(element, options, callback) {
    ...
    //var keyboardHandler = google.maps.event.addListener(thisMap, 'click', handleAddPointClick);
...
Mapifies.ClearMap = function(element, options, callback) {
	...
	for(var i= 0;i < listCircles.length; i++){
			listCircles[i].setMap(null);
		}
	listCircles.length=0;
	//clear polygons
	for(var i= 0;i < listPolygons.length; i++){
			listPolygons[i].setMap(null);
		}
	listPolygons.length=0;
    if (typeof callback == 'function') return callback(thisMap);
}
	...
	*/
	...
	if (typeof thisMap.Directions === 'undefined') {
		...
		var directionsRendererOptions = {
			...
			panel: panel,
			preserveViewport: options.preserveViewport,
			polylineOptions: options.getPolyline
		};
		Mapifies.MapObjects.Append(element, 'Directions', new google.maps.DirectionsRenderer(thisMap, panel));
	}
	...
	directionService.route(queryOptions, callback)
...
* @return {Function} callback The callback option with the feed object and options.
*/
Mapifies.AddRoundGroundOverlay = function(element, options, callback) {
    /**
    * Default options for AddGroundOverlay
    * @method
    * @namespace Mapifies.AddGroundOverlay
    ...
    */
	...
	var center = new google.maps.LatLng(options.mapCenter[0], options.mapCenter[1]);  
	var radius = options.radius;
	var circleRadius;
	if (radius != "") {
      circleRadius = parseFloat(radius);
      //cover mile to meter
      circleRadius = circleRadius * 1609.344
	} 
	else {
      circleRadius = 0;
	};
	fillColor = options.fillColor || options.liColor || "#0055ff";
	/*
    ...
    fillColor = options.fillColor || options.liColor || "#0055ff";
    liWidth = options.liWidth || 2;
    groundOverlay = new google.maps.Polygon(points, options.liColor, liWidth, null, fillColor, null);
    thisMap.addOverlay(groundOverlay);
    var zoomLevel = thisMap.getBoundsZoomLevel(bounds);
	...
	*/
	...
	thisMap.setCenter(circle.getCenter());
	thisMap.fitBounds(circle.getBounds());	
    if (typeof callback == 'function') return callback(groundOverlay, options);
    return;
};
...
* @return {Function} callback The callback option with the feed object and options.
*/
Mapifies.AddRoundCircleOverlay = function(element, options, callback) {
    ...
    bounds.extend(point);
        ...
        fillColor = options.fillColor || options.liColor || "#0055ff";
    liWidth = options.liWidth || 2;
    groundOverlay = new GPolygon(points, options.liColor, liWidth, null, fillColor, null, { clickable: false });
    thisMap.addOverlay(groundOverlay);
    var zoomLevel = thisMap.getBoundsZoomLevel(bounds);
    thisMap.setZoom(zoomLevel);
    ...
    */
    ...
    if (typeof callback == 'function') {    
        ...
        return callback(circle, options);
    } else {
        console.log("Can't callback " + callback);
    }
    return;
};
...
Mapifies.AddMarker = function(element, options, callback) {
    ...
    */
	...
	jQuery.extend(markerOptions, { 'map':thisMap });
	jQuery.extend(markerOptions, { 'position':new google.maps.LatLng(options.pointLatLng[0], options.pointLatLng[1]) });
    ...
    if (options.pointHTML)
        ...
        google.maps.event.addListener(marker, options.pointOpenHTMLEvent, function() {
            ...
            infoWindow.setContent(options.pointHTML);
...
Mapifies.SearchCode = function(code) {
    switch (code) {
        case "OK":
            return { 'code': "OK", 'success': true, 'message': 'The response contains a valid GeocoderResponse.' };
        case "OVER_QUERY_LIMIT":
            return { 'code': 'OVER_QUERY_LIMIT', 'success': false, 'message': 'The webpage has gone over the requests limit in too short a period of time.' };
            break;
        case "ERROR":
            return { 'code': 'ERROR', 'success': false, 'message': 'There was a problem contacting the Google servers.' };
            break;
        case "UNKNOWN_ERROR":
            return { 'code': 'UNKNOWN_ERROR', 'success': false, 'message': 'A geocoding request could not be processed due to a server error. The request may succeed if you try again.' };
            break;
        case "REQUEST_DENIED":
            return { 'code': 'REQUEST_DENIED', 'success': false, 'message': 'The webpage is not allowed to use the geocoder.' };
            break;
        case "INVALID_REQUEST":
            return { 'code': 'INVALID_REQUEST', 'success': false, 'message': 'This GeocoderRequest was invalid.' };
            break;
        case "ZERO_RESULTS":
            return { 'code': 'ZERO_RESULTS', 'success': false, 'message': 'No result was found for this GeocoderRequest.' };
            break;
        default:
            return {
                'code': null,
