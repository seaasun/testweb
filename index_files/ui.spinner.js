 ...
 *  ui.core.js
 *
 *  czhu 09/27/2009 updated: update _parse and _format function, supporting input null value
 *
 */
(function($) {
    ...
    $.widget('ui.spinner', {
        ...
        _draw: function() {
			...
			.bind('change' + namespace, function(event) {
			    self._change(event);
			})
        ...
        _parse: function(val) {
            ...
            if (typeof val == 'string' && val != "") {
                ...
                if (val.length > 1) {
                    ...
                    (val[0].toString().substring(0, 1) == '-' ? -1 : 1); //The orginal code has bug in IE 7.0, the val[0][0] can't get the first code "-"
                    //(val[0][0] == '-' ? -1 : 1);
                }
                val = result;
            }
            return ((typeof val == 'string' && val == "") || isNaN(val)) ? null : val;
        },
        _format: function(num) {
			...
			neg = num < 0 ? '-' : '',
            mindec = options.minPrecision,
            maxdec = options.precision;

            dec = mindec > 0 && mindec < maxdec ? mindec : maxdec;
            if (isNaN(num)) {
                // do nothing  dec=dec;
                dec = mindec;
            } else if (num.toString().indexOf('.') != -1 && mindec > 0 && mindec < maxdec) {
                var s = num.toString();
                var len = s.slice(s.indexOf('.') + 1, s.length).length;
                if (len <= mindec) {
                    dec = mindec;
                } else if (len < maxdec) {
                    dec = len;
